...
const VerticalStack = ({ header, footer, children }: StackProps) => {
  return (
    <div className="flex flex-col h-full w-full">
      ...
      {children}
      <div className="h-7 shrink-0 uppercase text-center">{footer}</div>
    </div>
  );
};
  ...
  return (
    <>
      <div className="flex flex-row h-full w-full gap-x-2">
        <div className="w-1/2 uppercase grid content-center truncate">
          {header}
        </div>
        <div className="w-1/2 shrink-0 flex">{children}</div>
      </div>
    </>
  );
};
